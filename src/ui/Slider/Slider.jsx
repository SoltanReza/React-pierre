...
const Slider = ({
  ...
  const onMouseUp = (e) => {
    ...
    if (walk < minWalk && walk > -minWalk)
      return (elm.scrollLeft =
        +(scrollLeft / offsetWidth).toFixed() * offsetWidth);
    if (startX > endX)
      return (elm.scrollLeft =
        (+(scrollLeft / offsetWidth).toFixed() + 1) * offsetWidth);
    if (startX < endX)
      return (elm.scrollLeft =
        (+(scrollLeft / offsetWidth).toFixed() - 1) * offsetWidth);
  };
  ...
  const sliderProps = {
    ref: slider,
    className: `${styles.root} position-relative ${className}`,
  };
  if (!showArrows) {
  ...
  return (
    <div className="position-relative" {...props}>
      ...
      {showArrows && (
        <>
          <button
            className="btn-icon bg-light position-absolute top-50 translate-middle-y end-0 h6 me-3 me-lg-5"
            onClick={onPrevious}
          >
            <i className="fas fa-arrow-left"></i>
          </button>
          <button
            className="btn-icon bg-light position-absolute top-50 translate-middle-y start-0 h6 ms-3 ms-lg-5"
            onClick={onNext}
          >
